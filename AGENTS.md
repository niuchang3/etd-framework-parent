...
- 当前项目处于底座搭建阶段，尚不存在需要维护的历史版本、存量部署或旧协议。除非用户明确提出兼容目标，否则禁止主动编写任何向后兼容、过渡兼容或推测性兼容代码。
- 禁止为了“以后可能需要”而保留旧实现、旧字段、旧表、旧配置键、旧接口、旧依赖或双套处理链路；目标方案确定后，应直接采用唯一实现并删除本次改动范围内已经失效的过渡代码。
- 禁止增加无实际需求的 fallback、别名映射、双读双写、旧数据兜底、废弃 API 包装、兼容分支、兼容性 SQL 以及仅服务于旧方案的依赖。
- 初始化 SQL 应表达当前底座的最终结构，不按数据库迁移脚本思路兼容尚未投入使用的旧结构；不得为了兼容不存在的历史环境而保留旧表、旧列或重复 DDL。
- 只有用户明确说明需要兼容的具体对象、版本或数据后，才允许实施兼容处理，并必须在代码注释和回复中说明兼容边界与删除条件。
- 本节是阶段性最高优先级规范。只有用户明确告知“底座搭建完成”或要求移除本规范后，AI 才能删除或停止执行本节规则，不得自行判断阶段已经结束。
...
## ShardingSphere-JDBC 使用规范

- 项目中的 `sharding-jdbc` 统一指 ShardingSphere-JDBC。分库分表规则必须优先使用官方 YAML 配置，不得把可声明在 YAML 中的标准分片策略改写为 Java 配置类或自定义算法。
- 优先使用 ShardingSphere 内置分片算法。只有内置算法无法满足已经确认的业务规则，且用户明确认可后，才允许新增自定义分片算法；不得为了封装简单取模、哈希或范围规则而增加 Java 扩展代码。
- ShardingSphere 配置必须按运行环境隔离：公共 `application.yml` 不得包含具体物理数据源；`application-{profile}.yml` 负责引用对应的 `sharding-{domain}-{profile}.yaml`，物理地址、账号和密码必须通过该环境的环境变量注入。
- 各环境的 ShardingSphere YAML 必须保持相同的逻辑表、分片键、算法名称和分片数量语义。环境文件只允许在物理连接信息、连接池容量、诊断开关等确有环境差异的配置上不同。
- ShardingSphere Driver 作为 Spring 的逻辑数据源入口时，外层使用 `org.apache.shardingsphere.driver.ShardingSphereDriver`；真实数据库连接池必须配置在 ShardingSphere YAML 的 `dataSources` 中，不得同时再配置另一套绕过 ShardingSphere 的业务数据源。
- 真实数据源必须和同类业务模块的连接池技术及参数风格保持一致。项目使用 Druid 时，YAML 中必须使用 `com.alibaba.druid.pool.DruidDataSource` 及其正确属性名，不得混入 Hikari 专用属性、适配依赖或无用连接池实现。
- 分片表的实体、SQL、Mapper 查询条件和关联查询必须正确携带分片键，避免无意触发全路由。需要经常关联的分片表应使用相同分片键和算法，并在规则中声明绑定表。
- 不分片但由同一逻辑数据源管理的表，必须明确配置为单表；不得依赖运行时自动猜测或为了能启动而增加模糊兜底规则。
- 初始化 SQL 中的物理表名称、数量、约束和索引必须与 YAML 的实际节点及分片数量完全一致。调整分片数量或算法时，必须同步检查建表 SQL、测试和运维说明。
- ShardingSphere 相关依赖必须保持最小化。新增或移除能力模块后必须检查 dependency tree，禁止保留当前配置未使用的数据库方言、连接池适配器、自定义算法或兼容依赖。
- 修改 ShardingSphere 配置后，至少要增加或更新一个真实加载 YAML、创建逻辑数据源并校验路由所需物理表的聚焦测试；不得只验证 YAML 语法或仅凭配置文件外观判断可用。
...
- **AI 新增或修改的代码必须包含清晰、准确且有实际说明价值的中文注释，此要求为强制规范，不得省略**：
  - 新增类、接口、注解、枚举、Record、配置类、切面和关键方法必须编写中文 JavaDoc；
  - DTO、VO、Entity 中声明的每一个属性都必须紧邻编写中文 JavaDoc，准确说明字段的业务含义、单位、格式、取值约束或协议语义；不得只给类写注释而省略属性注释，也不得用字段名的机械翻译敷衍。继承自基类的属性无需在子类重复声明或重复注释；
  - 对外暴露的公共 API 必须使用中文 JavaDoc 说明用途、参数、返回值、异常以及关键使用约束；
  - 配置项、协议字段、注解属性和具有业务或技术约定含义的常量必须使用中文注释说明作用、默认行为及必要限制；
  - 复杂业务规则、关键分支、算法步骤、非显而易见的处理、上下文传递、异步边界、事务边界、兼容性决策和缓存失效原因必须添加简洁的中文说明；
  - 修改已有代码时，如果被修改区域缺少必要的中文注释，应在本次改动范围内一并补齐；
  - 提交前必须主动检查本次新增和修改的代码是否满足中文注释要求；
